...
全库扫描
  ...
  → 按分排序，取 pool_size（检索器内部截断，默认 30；规划参考路会放大以便抽够条数）
...
**规划官参考量**（`retrieved_material`）：默认主问 8 道 ×4 = **32** 条真题，A/B 各 16，无场景路时 32 条全进 A。随 `question_count` 放大，上限 48。这些题写入规划官 Prompt，**只作出题参考**（改写、交叉、定考点），不是候选池、不是让规划官从中勾 8 道原题，也不是候选人听到的题单。
